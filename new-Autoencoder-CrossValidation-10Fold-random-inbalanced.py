...
# Hyperparameter tuning for Autoencoder with Random Search
def random_search_autoencoder_hyperparameters(X, y, n_iter=10):
    best_params = None
    best_metrics = None
    best_f1_score = -float('inf')

    for _ in range(n_iter):
        encoding_dim = random.choice([8, 16, 32, 64])
        epochs = random.choice([30, 50, 70])
        batch_size = random.choice([16, 32, 64])

        print(f"Testing encoding_dim={encoding_dim}, epochs={epochs}, batch_size={batch_size}")
...
output_file_path = sys.argv[2] + "/autoencoder_inbalanced_random.txt"
#output_file_path = sys.argv[2] + "/autoencoder_over_random.txt"
#output_file_path = sys.argv[2] + "/autoencoder_under_random.txt"
#output_file_path = sys.argv[2] + "/autoencoder_hybrid_random.txt"
...
if __name__ == "__main__":
    ...
    email_subject = (f"Script Completion Notification - Autoencoder 10-Fold Cross Validation (Imbalaced) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - Autoencoder 10-Fold Cross Validation (Over) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - Autoencoder 10-Fold Cross Validation (Under) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - Autoencoder 10-Fold Cross Validation (Hybrid) - Random - {save_directory}")
    ...
    # Perform Random Search for Autoencoder hyperparameters
    best_params, best_metrics = random_search_autoencoder_hyperparameters(X, y, n_iter=20)
