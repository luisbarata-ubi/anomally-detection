...
def evaluate_model(params, X, y):
    ...
    """
    # Instantiate GaussianNB with the chosen parameter(s)
    ...
    for train_index, test_index in kf.split(X):
        ...
        # Split training data into (train + validation) and test fold
...
def random_search(param_space, n_iter, X, y):
    """
    param_space should be a dict of lists, e.g.:
    {
    ...
    for i in range(n_iter):
        # Randomly choose hyperparameters from the lists in param_space
        params = {
            'var_smoothing': random.choice(param_space['var_smoothing'])
        }

        metrics = evaluate_model(params, X, y)

        # We track the best parameters based on F1 score
...
output_file_path = sys.argv[2] + "/gaussian_inbalanced_random.txt"
#output_file_path = sys.argv[2] + "/gaussian_over_random.txt"
#output_file_path = sys.argv[2] + "/gaussian_under_random.txt"
#output_file_path = sys.argv[2] + "/gaussian_hybrid_random.txt"
...
if __name__ == "__main__":
    ...
    email_subject = (f"Script Completion Notification - GaussianNB 10-Fold Cross Validation (Imbalaced) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - GaussianNB 10-Fold Cross Validation (Over) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - GaussianNB 10-Fold Cross Validation (Under) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - GaussianNB 10-Fold Cross Validation (Hybrid) - Random - {save_directory}")
    ...
    # Define your parameter space for random search (GaussianNB)
    param_space = {
        # Typically, var_smoothing ranges around 1e-9; adjust the list to your preference
