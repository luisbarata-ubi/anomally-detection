...
def random_search(param_space, n_iter, X, y):
    """
    Randomly searches hyperparameters from `param_space` for a total of `n_iter` iterations.
    param_space is a dict of lists, e.g.:
    {
        'n_neighbors': [3, 5, 7, 9],
    ...
    for i in range(n_iter):
        # Randomly pick hyperparameters from each list
        params = {
            'n_neighbors': random.choice(param_space['n_neighbors']),
            'weights': random.choice(param_space['weights']),
            'p': random.choice(param_space['p'])
        }

        metrics = evaluate_model(params, X, y)
        # Track the best parameters based on F1 score
...
#output_file_path = sys.argv[2] + "/knn_inbalanced_random.txt"
#output_file_path = sys.argv[2] + "/knn_over_random.txt"
output_file_path = sys.argv[2] + "/knn_under_random.txt"
#output_file_path = sys.argv[2] + "/knn_hybrid_random.txt"
...
if __name__ == "__main__":
    ...
    #email_subject = (f"Script Completion Notification - KNN 10-Fold Cross Validation (Imbalaced) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - KNN 10-Fold Cross Validation (Over) - Random - {save_directory}")
    email_subject = (f"Script Completion Notification - KNN 10-Fold Cross Validation (Under) - Random - {save_directory}")
    #email_subject = (f"Script Completion Notification - KNN 10-Fold Cross Validation (Hybrid) - Random - {save_directory}")
