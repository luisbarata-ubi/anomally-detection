...
def objective(params):
    # Initialize the Elliptic Envelope model with current hyperparameters
    robust_cov_model = EllipticEnvelope(
    ...
    for train_index, test_index in kf.split(X):
        ...
        y_val_pred = robust_cov_model.predict(X_val)
        y_test_pred = robust_cov_model.predict(X_test_fold)

        # EllipticEnvelope returns -1 for anomalies and 1 for normal points, convert this to 0 (normal) and 1 (anomaly)
        y_val_pred = np.where(y_val_pred == -1, 1, 0)
        y_test_pred = np.where(y_test_pred == -1, 1, 0)

        # Calculate train and test accuracies
        train_accuracy = accuracy_score(y_train, robust_cov_model.predict(X_train))
...
space = {
    ...
    'support_fraction': hp.uniform('support_fraction', 0.1, 1.0)
}

# Redirect print statements to a file
#output_file_path = sys.argv[2] + "/robustcovariance_inbalanced_hyperopt.txt"
#output_file_path = sys.argv[2] + "/robustcovariance_over_hyperopt.txt"
#output_file_path = sys.argv[2] + "/robustcovariance_under_hyperopt.txt"
output_file_path = sys.argv[2] + "/robustcovariance_hybrid_hyperopt.txt"
...
if __name__ == "__main__":
    ...
    #email_subject = (f"Script Completion Notification - Robust Covariance 10-Fold Cross Validation (Imbalaced) - HyperOPT - {save_directory}")
    #email_subject = (f"Script Completion Notification - Robust Covariance 10-Fold Cross Validation (Over) - HyperOPT - {save_directory}")
    #email_subject = (f"Script Completion Notification - Robust Covariance 10-Fold Cross Validation (Under) - HyperOPT - {save_directory}")
    email_subject = (f"Script Completion Notification - Robust Covariance 10-Fold Cross Validation (Hybrid) - HyperOPT - {save_directory}")
